...
function ping() {
    echo "$output" "$list" "$total" "$count" "$min" "$mintxt" "$max" "$maxtxt"
    ...
    then
        ...
        then
            declare -g min=$ping
            declare -g mintxt="$output"
        fi
        if (( $(echo "$ping > $max" | bc -l) ))
        then
            declare -g max=$ping
            declare -g maxtxt="$output"
        fi
    fi
}
...
while read output
do
    ping "$output" "$list" "$total" "$min" "$mintxt" "$max" "$maxtxt" && count=$(echo "$count+1" | bc) &
